...
namespace Vulkan
{
	...
	{
	public:
		explicit Version(const uint32_t version) : Major(VK_VERSION_MAJOR(version)),
												   Minor(VK_VERSION_MINOR(version)),
												   Patch(VK_VERSION_PATCH(version))
		{
		}

		Version(const uint32_t version, const uint32_t vendorId) : Major(VK_VERSION_MAJOR(version)),
